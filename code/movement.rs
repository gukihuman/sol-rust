...
impl Plugin for MotionPlugin { fn build(&self, app: &mut App) { app
    .init_resource::<ControlledEntity>()
    .add_systems(Update, (
        update_destination,
        ...
        if let Ok((transform, mut movement)) = query.get_mut(entity) {
            ...
            if gamepad_state.left_stick_deadzone_exceed {
                let change_in_direction = Vec2::new(
                    gamepad_state.left_stick_x, gamepad_state.left_stick_y
                ) * movement.speed;
...
pub fn update_position(
    ...
    for (mut transform, mut movement) in query.iter_mut() {
        movement.velocity_desire = calculate_velocity_desire(
            &transform, &mut movement
        );
        movement.velocity = calculate_velocity(
            &mut movement, delta_seconds
        );
...
fn draw_gizmos(
    ...
    if let Some(entity_id) = controlled_entity.0 {
        if let Ok((transform, movement)) = query.get(entity_id) {
            ...
            gizmos.line(transform.translation, velocity_desire, Color::GREEN);
            ...
            if let Some(destination) = &movement.destination {
                gizmos.circle_2d(*destination, 1.0, Color::GREEN);
            }
        }
    }
}