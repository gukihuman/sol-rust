...
} impl Default for Movement { fn default() -> Self { Self {
    speed: 100.,
...
pub fn update_go(
    ...
    for (mut transform, mut movement) in query.iter_mut() {
        if let Some(destination) = movement.destination {
            let dir_to_dest = destination - Vec2::new(
                transform.translation.x, transform.translation.y
            );
            let desired_direction =
                if dir_to_dest.length() == 0.0 { Vec2::default() }
                else { dir_to_dest.normalize() };
            let distance_to_destination = dir_to_dest.length();
            if distance_to_destination < 1.0 { movement.destination = None; }
            let dynamic_speed =
                if distance_to_destination < movement.close_enough {
            ...
            let adjustment = (desired_velocity - movement.current_velocity) *
                movement.velocity_change_rate;
            movement.current_velocity =
                movement.current_velocity + adjustment * delta_seconds;
        ...
        } else if movement.current_velocity.length() > 1.0 {
            let adjustment =
                -movement.current_velocity * movement.velocity_change_rate;
            movement.current_velocity =
                movement.current_velocity + adjustment * delta_seconds;

            transform.translation.x +=
                movement.current_velocity.x * delta_seconds;
            transform.translation.y +=
                movement.current_velocity.y * delta_seconds;
        }
    }
}
pub fn update_destination(
    ...
    if let Some(entity) = controlled_entity.0 {
        if let Ok((transform, mut movement)) = query.get_mut(entity) {
            ...
            } else { movement.destination = None }
        }
    }
}
